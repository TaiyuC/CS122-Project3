...
class World:
    ...
    def glimpse_monster(self):
        """Only see the monster with not a lot details"""
        ret_list = []
    ...
    def view_monster(self):
        """gives the player the chance to see how many monsters in each types of land (as dictionary), and see the names of monsters (as list)"""
        ret_dict = {}
        ret_list = []
        for i in range(0, self.gridsize):
            for j in range(0, self.gridsize):
    ...
    def get_desert_loc(self):
        """Get the locations of dessert so we can add a stronger monster in the room that has acient core"""
        desert_loc = []
        for i in range(0, self.gridsize):
            for j in range(0, self.gridsize):
    ...
    def initialize_item(self):
        """randomly create items and a monster in the desert that has a core"""
        for i in range(0, self.gridsize):
            for j in range(0, self.gridsize):
    ...
    def item_new(self, n = 1):
        """create new items to where monsters move around, default is create one item."""
        for _ in range(n):
            loc1, loc2 = self.monster_loc()
            self.item_by_roomtype(loc1, loc2)
    
    def create_mon(self, loc1, loc2):
        """create different types of monsters, most names comes from the acient book 'Classic of Mountains and Seas'."""
    ...
    def mon_new(self, n = 1):
        """create random monsters to random location, the default is add one monster"""
        for _ in range(n):
            loc1, loc2 = self.monster_loc()
            self.create_mon(loc1, loc2)
